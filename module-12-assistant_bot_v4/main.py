...
# ---------
    ...
    def show(self, name):
        ...
        for count, value in enumerate(self.data[name].phones):
            ...
            result += '-'*maxlen+'\n'
        result += ('|  {:^10}  |  {:^'+str(maxlen-21)+'}  |\n').format('birthday', self.data[name].birthday.value)
        result += '-'*maxlen+'\n'
...
# module12 homework --- search
    def search(self, search_string):
        result = ''
        for contact in self.data:
            if contact.count(search_string):
                result += self.show(contact)+'\n'
            else:
                for phone in self.data[contact].phones:
                    if phone.value.count(search_string):
                        result += self.show(contact)+'\n'
                        break
        return result
#----------------
...
@input_error
def search(args_string):
    args = processing_args(args_string, 1, 1, 'use:\n   search <search string>')
    return contacts.search(*args)
...
FUNC_DICT=  {'hello': hello, 'add': add, 'change': change, 'phone': phone,
            ...
            'show': show, 'remove': remove, 'birthday': birthday, 'search': search}
#contacts = AddressBook(dict())

# module12 homework --- load data
try:
